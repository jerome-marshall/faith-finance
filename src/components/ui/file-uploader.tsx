...
import { Plus, Trash2 as RemoveIcon, Replace, Trash2 } from "lucide-react";
...
export const ImageUploadActions = forwardRef<
  HTMLDivElement,
  React.HTMLAttributes<HTMLDivElement>
>(({ className, ...props }, ref) => {
  const { removeFileFromSet, dropzoneState } = useFileUpload();

  const [file, setFile] = useState(dropzoneState.acceptedFiles[0]);

  useEffect(() => {
    if (dropzoneState.acceptedFiles[0]) {
      setFile(dropzoneState.acceptedFiles[0]);
    }
  ...
  const ImageActionItems = [
    {
      icon: (
        <FileInput className="flex size-full items-center justify-center ">
          {file ? <Replace size={20} /> : <Plus size={20} />}
        </FileInput>
      ),
      onClick: () => null,
      alt: file ? "Replace Image" : "Add Image",
      id: "image-action-add",
    },
    ...(file
      ? [
